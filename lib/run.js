  ...
  _initialize({ up, down }) {
    try {
      ...
      this.up = this.mongoClient.connect(up, { useNewUrlParser }).then(res => {
        console.log(` Database connection to ${res.db().databaseName} database was established ✓`.underline.magenta)
        return res
      })
      this.down = this.mongoClient.connect(down, { useNewUrlParser }).then(res => {
        console.log(` Database connection to ${res.db().s.databaseName} database was established ✓`.underline.magenta)
        return res
      })
    } catch (err) {
      this._handle(err)
